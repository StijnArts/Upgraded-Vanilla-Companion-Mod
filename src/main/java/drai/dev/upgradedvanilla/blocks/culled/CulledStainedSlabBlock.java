...
public class CulledStainedSlabBlock extends SlabBlock implements BeaconBeamBlock {
	private final DyeColor color;

	public CulledStainedSlabBlock(DyeColor color, BlockBehaviour.Properties settings) {
		super(settings);
		this.color = color;
	}
	...
	public boolean skipRendering(BlockState blockState_1, BlockState blockState_2, Direction direction_1) {
		if (blockState_2.getBlock() instanceof StainedGlassBlock) {
			...
			return blockState_2.getBlock() instanceof CulledStainedStairBlock && ((CulledStainedStairBlock)blockState_2.getBlock()).getColor() == this.color && this.isInvisibleToGlassStairs(blockState_1, blockState_2, direction_1) ? true : super.skipRendering(blockState_1, blockState_2, direction_1);
		}
	}

...
public class SpreadableWallBlock extends BaseDirtWall implements SimpleWaterloggedBlock {
	public static final BooleanProperty SNOWY;
	private BaseDirtWall baseDirtWall;
	private SpreadingSnowyDirtBlock originBlock;

	public SpreadableWallBlock(SpreadingSnowyDirtBlock originBlock, BaseDirtWall baseDirtWall) {
		super(FabricBlockSettings.copy(originBlock));
		this.baseDirtWall = baseDirtWall;
	...
	public void randomTick(BlockState state, ServerLevel level, BlockPos pos, RandomSource random) {
		if (!canSurvive(state, level, pos)) {
			if (baseDirtWall !=null) {
				level.setBlock(pos, baseDirtWall.withPropertiesOf(level.getBlockState(pos)), 2);
			} else {
